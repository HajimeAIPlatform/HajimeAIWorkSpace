...
func (dc *DifyClient) UploadDatasetsByUploadFile(datasets_ids []string,datasets_id string) (result InitDatasetsResponse, err error) {
	payload := &InitDatasetsPayload{
		DocForm:           "text_model",
		DocLanguage:       "Chinese",
		IndexingTechnique: "high_quality",
...
func (dc *DifyClient) DocumentIndexingStatus(datasets_id string,document_id string) (result InitDatasetsIndexingStatusData, err error) {
	api := dc.GetConsoleAPI(CONSOLE_API_DOCUMENT_STATUS)
	api = UpdateAPIParam(api, CONSOLE_API_PARAM_DATASETS_ID, datasets_id)
